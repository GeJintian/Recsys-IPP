...
class DQNAgent(Agent):
    ...
    def _build_model(self):
        ...
        model = Sequential()
        model.add(Masking(mask_value=0., input_shape=(200, self.input_dim + 1)))
        model.add(LSTM(200))
...
# Reset env and set done to False.
    ...
    current_state = np.zeros((1,200,num_products+1))
    next_state = np.zeros((1,200,num_products+1))
    action = None
    count_step = 0
    #print('s1')
    if observation:
    ...
    while not done:
        ...
        if action:
            if reward == 0:
                ...
                next_state[0,count_step,num_products] = 1
            if reward == 1:
                next_state[0,count_step,action] = 1
                next_state[0,count_step,num_products] = 1
